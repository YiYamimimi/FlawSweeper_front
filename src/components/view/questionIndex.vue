...
<template>
  <div>
    <!-- 搜索框 -->
    <div id="wrap">
      <el-button id="back" plain @click="returnPage">返回上一级</el-button>
...
<script>
import showboxVue from "./showbox.vue";
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      this.$axios
        .get("/question/pagebyclassify" + this.addPath)
        .then((res) => {
    ...
    titleDate() {
      this.$axios
        .get("/search/selectbytitle" + this.addPath2)
        .then((res) => {
    ...
    noteDate() {
      this.$axios
        .get("/search/selectbynote" + this.addPath2)
        .then((res) => {
          console.log(res);
          this.questionList = res.data.data.row;
          this.total = res.data.data.totalCount;
  ...
  computed: {
    addPath() {
      return "/" + this.$route.query.id + "/" + this.page + "/" + this.limit;
    },
    addPath2() {
      return (
        "/" +
