...
<script>
import showboxVue from "../recycleBin/recycleshowbox.vue";
export default {
  components: {
    showboxVue,
  },
  ...
  methods: {
    ...
    fetchData() {
      this.$axios
        .get("/question/pagebyrecycle" + this.addPath)
        .then((res) => {
    ...
    titleDate() {
      this.$axios
        .get("/search/selectrecyclebytitle" + this.addPath2)
        .then((res) => {
          this.questionList = res.data.data.row;
          this.total = res.data.data.totalCount;
        })
    ...
    noteDate() {
      this.$axios
        .get("/search/selectrecyclebynote" + this.addPath2)
        .then((res) => {
          console.log(res);
          this.questionList = res.data.data.row;
          this.total = res.data.data.totalCount;
  ...
  computed: {
    ...
    addPath2() {
      return "/" + this.searchText + "/" + this.page + "/" + this.limit;
    },
  },
};
</script>
